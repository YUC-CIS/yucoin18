...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete yucoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no yucoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old yucoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected yucoin 0.7 datadir."
    ;;
  3)
    echo "Detected yucoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected yucoin 0.8 datadir."
    ;;
esac
